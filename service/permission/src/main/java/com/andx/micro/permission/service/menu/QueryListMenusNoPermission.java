 ...
 */
@Component
@Service(path = "/list-menus/noPermission", code = "queryListMenusNoPermission", name = "查询菜单列表(无权限)", module = "permission")
public class QueryListMenusNoPermission extends GetSampleService<Response<List<MenuDto>>> {
    ...
    private void fetch(List menuDtos, List<Menu> menus) {
        for (Menu sonMenu : menus) {
            ...
            if (permission == null || sonMenu.getSons().size() > 0) {
